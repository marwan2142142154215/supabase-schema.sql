...
import {
  ...
  MessageSquare, Image, History, LogOut, Save, Plus, ToggleLeft, ToggleRight, Key, Lock
} from 'lucide-react'
import { formatTime } from '../utils/helpers'
import { ALL_PERMISSIONS } from '../lib/permissions'

const sections = [
  { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, key: 'access_master_panel' },
  { id: 'staff', label: 'Manajemen Staff', icon: Users, key: 'manage_staff' },
  { id: 'shifts', label: 'Pengaturan Shift', icon: Clock, key: 'manage_shifts' },
  { id: 'durations', label: 'Durasi Izin', icon: Timer, key: 'manage_roles' },
  { id: 'roles', label: 'Role & Kuota', icon: UserCog, key: 'manage_roles' },
  { id: 'permissions', label: 'Hak Akses', icon: Key, key: 'manage_permissions' },
  { id: 'emergency', label: 'Emergency & Reset', icon: AlertTriangle, key: 'emergency_stop' },
  { id: 'regulations', label: 'Peraturan', icon: BookOpen, key: 'manage_regulations' },
  { id: 'telegram', label: 'Integrasi Telegram', icon: MessageSquare, key: 'manage_telegram' },
  { id: 'appearance', label: 'Tampilan', icon: Image, key: 'manage_appearance' },
  { id: 'security', label: 'Keamanan', icon: Lock, key: 'manage_security' },
  { id: 'photos', label: 'Manajemen Foto', icon: Image, key: 'manage_photos' },
  { id: 'logs', label: 'Riwayat & Log', icon: History, key: 'access_master_panel' },
]
...
export default function MasterPanel() {
  ...
  // Check if user has master panel access
  useEffect(() => {
    if (!user || !canAccess('access_master_panel')) {
      if (user) { navigate('/'); toast.error('Akses ditolak!') }
    }
  }, [user])
    ...
    return (
      <div className="min-h-screen bg-primary flex items-center justify-center p-4">
        <div className="text-center">
          <Shield size={48} className="mx-auto text-accent-red mb-4" />
          <h2 className="text-xl font-bold mb-2">Akses Ditolak</h2>
          <p className="text-secondary">Anda tidak memiliki izin untuk mengakses Master Panel.</p>
        </div>
      </div>
    )
  }
  ...
  const [keluarDur, setKeluarDur] = useState(15)
  ...
  const fetchAll = useCallback(async () => {
    const [sRes, rRes, shRes, cRes, bRes, lRes, regRes, rpRes] = await Promise.all([
    ...
    if (rpRes.data) {
      ...
      rpRes.data.forEach((rp: any) => {
        ...
        permMap[rp.role_id].push(rp.permission_key)
      })
      setRolePermissions(permMap)
    }
    const durs = cRes.data?.find((c: any) => c.key === 'break_durations')?.value || { keluar: 15, meal: 8 }
    setKeluarDur(durs.keluar); setMealDur(durs.meal)
    setTolerance(cRes.data?.find((c: any) => c.key === 'late_tolerance_seconds')?.value || 59)
  }, [])

  useEffect(() => { fetchAll(); const sub = supabase.channel('master-panel').on('postgres_changes', { event: '*', schema: 'public' }, fetchAll).subscribe(); return () => { supabase.removeChannel(sub) } }, [fetchAll])
  ...
  const handleLogin = async () => {
    ...
    if (password === (config.admin_password || 'nasdes')) {
      ...
      setAttempts(newAttempts); securityGuard.recordFailedLogin('master-panel')
      ...
      <div className="min-h-screen bg-primary flex items-center justify-center p-4">
        <div className="bg-secondary rounded-2xl p-8 border border-custom max-w-sm w-full">
          ...
          <div className="space-y-4">
            ...
            {lockout > Date.now() && <p className="text-accent-red text-sm text-center">Lockout: {Math.ceil((lockout - Date.now()) / 1000)}s</p>}
  ...
  const filteredSections = sections.filter(s => canAccess(s.key as any))
    ...
    <div className="flex min-h-screen bg-primary">
      <aside className="w-64 bg-secondary border-r border-custom fixed h-full overflow-y-auto z-40">
        <div className="p-4 border-b border-custom">
          ...
          <p className="text-xs text-secondary">Master Panel • {user.roleName}</p>
        </div>
        ...
        <div className="p-4 border-t border-custom">
          <button onClick={() => { logout(); navigate('/') }} className="flex items-center gap-2 text-accent-red text-sm hover:bg-accent-red/10 w-full px-3 py-2 rounded-xl">
      ...
      <main className="flex-1 ml-64 p-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="bg-card rounded-2xl p-5 border border-custom">
                ...
                <p className="text-sm text-secondary">Total Role</p>
              </div>
            </div>
          </div>
        )}
        ...
        {/* STAFF MANAGEMENT */}
        {activeSection === 'staff' && canAccess('manage_staff') && (
          <div className="space-y-6">
            <h1 className="text-2xl font-bold">Manajemen Staff</h1>
            <div className="bg-card rounded-2xl p-4 border border-custom">
              <div className="flex gap-2 flex-wrap">
                <input value={newStaffName} onChange={(e) => setNewStaffName(e.target.value)} placeholder="Nama Staff" className="flex-1 min-w-[150px] bg-secondary border border-custom rounded-xl px-4 py-2 text-sm text-white outline-none focus:border-accent-indigo" />
                ...
                }} className="px-4 py-2 bg-accent-indigo rounded-xl text-sm font-semibold"><Plus size={16} className="inline" /> Tambah</button>
              </div>
            </div>
                  ...
                  {staffList.map(s => (
                    <tr key={s.id} className="border-b border-custom/50 hover:bg-white/5">
                      ...
                      <td className="p-3 flex gap-1">
                        {editStaff?.id === s.id ? (
                          <><button onClick={async () => { await supabase.from('staff').update({ name: editStaff.name, role_id: editStaff.role_id }).eq('id', editStaff.id); toast.success('Staff diperbarui!'); setEditStaff(null); fetchAll() }} className="px-2 py-1 bg-accent-green rounded-lg text-xs">Simpan</button><button onClick={() => setEditStaff(null)} className="px-2 py-1 bg-card rounded-lg text-xs">Batal</button></>
                        ) : (
                          <><button onClick={() => setEditStaff({ id: s.id, name: s.name, role_id: s.role_id })} className="px-2 py-1 bg-accent-indigo/20 text-accent-indigo rounded-lg text-xs">Edit</button><button onClick={async () => { await supabase.from('staff').update({ active: !s.active }).eq('id', s.id); toast.success('Status staff diubah!'); fetchAll() }} className="px-2 py-1 bg-card rounded-lg text-xs">{s.active ? 'Nonaktifkan' : 'Aktifkan'}</button></>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        {/* SHIFTS */}
        {activeSection === 'shifts' && canAccess('manage_shifts') && (
          <div className="space-y-6">
            ...
            <div className="bg-card rounded-2xl p-4 border border-custom">
              <div className="flex gap-2">
                ...
                }} className="px-4 py-2 bg-accent-indigo rounded-xl text-sm"><Plus size={16} /></button>
              </div>
            </div>
            <div className="space-y-2">
              {shifts.map(s => (
                <div key={s.id} className="bg-card rounded-xl p-4 border border-custom flex items-center justify-between">
                  <div><p className="font-medium">{s.name}</p><p className="text-xs text-secondary">Cutoff: {s.cutoff_time}</p></div>
                  <button onClick={async () => { await supabase.from('shifts').update({ active: !s.active }).eq('id', s.id); fetchAll() }} className={`p-2 rounded-lg ${s.active ? 'text-accent-green' : 'text-gray-500'}`}>
        ...
        {/* DURATIONS */}
        {activeSection === 'durations' && canAccess('manage_roles') && (
          <div className="space-y-6">
            <h1 className="text-2xl font-bold">Durasi Izin</h1>
            <div className="bg-card rounded-2xl p-6 border border-custom space-y-4">
              {[
                { label: 'Izin Keluar (menit)', val: keluarDur, set: setKeluarDur },
                { label: 'Makan (menit)', val: mealDur, set: setMealDur },
              ].map(item => (
                <div key={item.label} className="flex items-center gap-4">
                  <label className="w-44 text-sm">{item.label}</label>
              ...
              <div className="flex items-center gap-4">
                <label className="w-44 text-sm">Toleransi (detik)</label>
              ...
              <button onClick={async () => {
                await supabase.from('system_config').upsert({ key: 'break_durations', value: { keluar: keluarDur, meal: mealDur } }, { onConflict: 'key' })
              ...
              }} className="px-6 py-3 bg-accent-indigo rounded-xl font-semibold flex items-center gap-2"><Save size={16} /> Simpan</button>
            </div>
          </div>
        )}

        {/* ROLES */}
        {activeSection === 'roles' && canAccess('manage_roles') && (
          <div className="space-y-6">
            ...
            <div className="bg-card rounded-2xl p-4 border border-custom space-y-3">
              <div className="grid grid-cols-6 gap-2">
                ...
                <input type="number" value={newRoleLong} onChange={(e) => setNewRoleLong(+e.target.value)} placeholder="Kuota Izin Keluar" className="bg-secondary border border-custom rounded-xl px-3 py-2 text-sm text-white" />
                ...
                <button onClick={async () => {
                  ...
                  await supabase.from('roles').insert({ name: newRoleName.trim(), max_concurrent: newRoleMax, quota_break_long_per_day: newRoleLong, quota_break_short_per_day: newRoleShort })
                  toast.success('Role ditambahkan!'); setNewRoleName(''); fetchAll()
                }} className="px-3 py-2 bg-accent-indigo rounded-xl text-sm"><Plus size={16} /></button>
              </div>
            </div>
            <div className="space-y-2">
              {roles.map(r => (
                <div key={r.id} className="bg-card rounded-xl p-4 border border-custom">
                  ...
                  <p className="text-xs text-secondary">Maks: {r.max_concurrent} | Izin Keluar: {r.quota_break_long_per_day}x | Makan: {r.quota_break_short_per_day}x</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeSection === 'permissions' && canAccess('manage_permissions') && (
          <div className="space-y-6">
            <h1 className="text-2xl font-bold">Hak Akses (Role-Based)</h1>
            <p className="text-sm text-secondary">Atur permission untuk setiap role. Hanya LEADER yang bisa mengelola ini.</p>
            <div className="flex gap-2 flex-wrap">
              {roles.map(r => (
                <button key={r.id} onClick={() => setSelectedRoleForPerms(r.id)}
                  className={`px-4 py-2 rounded-xl text-sm ${selectedRoleForPerms === r.id ? 'bg-accent-indigo text-white' : 'bg-card text-secondary hover:text-white'}`}>
                  {r.name}
                </button>
              ))}
            </div>
            {selectedRoleForPerms && (
              <div className="space-y-2">
                {ALL_PERMISSIONS.map(p => {
                  const hasPerm = rolePermissions[selectedRoleForPerms]?.includes(p.key)
                  return (
                    <div key={p.key} className="bg-card rounded-xl p-4 border border-custom flex items-center justify-between">
                      <div>
                      ...
                      <button onClick={async () => {
                        if (hasPerm) {
                          await supabase.from('role_permissions').delete().eq('role_id', selectedRoleForPerms).eq('permission_key', p.key)
                        } else {
                          await supabase.from('role_permissions').insert({ role_id: selectedRoleForPerms, permission_key: p.key })
                        }
                        fetchAll()
                        if (user?.roleId === selectedRoleForPerms) {
                          // Refresh own permissions if changing own role
                        }
                        toast.success(`Permission ${hasPerm ? 'dicabut' : 'diberikan'}!`)
                      }} className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${hasPerm ? 'bg-accent-green/20 text-accent-green' : 'bg-card text-secondary'}`}>
                        {hasPerm ? '✓ Aktif' : 'Nonaktif'}
                      </button>
                    </div>
                  )
                })}
        ...
        {/* EMERGENCY */}
        {activeSection === 'emergency' && canAccess('emergency_stop') && (
          <div className="space-y-6">
              ...
              <div className="space-y-2">
                {activeBreaks.map(b => (
                  <div key={b.id} className="bg-card rounded-xl p-4 border border-custom flex items-center justify-between">
                    ...
                    }} className="px-3 py-1.5 bg-accent-red/20 text-accent-red rounded-xl text-sm">Emergency Stop</button>
                  </div>
                ))}
              </div>
            )}
            <button onClick={async () => {
              ...
              for (const b of activeBreaks) { await supabase.from('break_records').update({ status: 'FORCE_STOPPED', return_time: new Date().toISOString() }).eq('id', b.id) }
              toast.success('Semua izin dihentikan!'); fetchAll()
            }} className="w-full py-4 bg-accent-red hover:bg-accent-red/80 text-white rounded-2xl font-bold text-lg">MATIKAN SEMUA IZIN AKTIF</button>
          </div>
        )}

        {/* REGULATIONS */}
        {activeSection === 'regulations' && canAccess('manage_regulations') && (
          <div className="space-y-6">
            ...
            <button onClick={async () => { await supabase.from('regulations').insert({ content: regContent.trim() }); toast.success('Peraturan diperbarui!') }} className="px-6 py-3 bg-accent-indigo rounded-xl font-semibold"><Save size={16} className="inline mr-2" /> Simpan</button>
          </div>
        )}

        {/* TELEGRAM */}
        {activeSection === 'telegram' && canAccess('manage_telegram') && (
          <div className="space-y-6">
            <h1 className="text-2xl font-bold">Integrasi Telegram</h1>
            <div className="bg-card rounded-2xl p-6 border border-custom space-y-4">
              <label className="flex items-center gap-3"><input type="checkbox" checked={tgEnabled} onChange={(e) => setTgEnabled(e.target.checked)} className="w-4 h-4 accent-accent-indigo" /><span className="text-sm">Aktifkan Notifikasi Telegram</span></label>
              ...
              }} className="px-6 py-3 bg-accent-indigo rounded-xl font-semibold"><Save size={16} className="inline mr-2" /> Simpan</button>
            </div>
          </div>
        )}

        {/* SECURITY */}
        {activeSection === 'security' && canAccess('manage_security') && (
          <div className="space-y-6">
            ...
            <div className="bg-card rounded-2xl p-6 border border-custom space-y-4">
              ...
              }} className="px-6 py-3 bg-accent-indigo rounded-xl font-semibold">Ganti Password</button>
            </div>
          </div>
        )}
        ...
        {(activeSection === 'appearance' || activeSection === 'photos') && (
          <div className="text-center py-12 text-secondary"><p>Fitur ini akan segera hadir.</p></div>
        )}
      </main>
    </div>
  )
